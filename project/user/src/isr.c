...
extern rt_sem_t bluetooth_sem;

void LPUART1_IRQHandler(void)//finsh
{
...
void LPUART3_IRQHandler(void)//openart1
{
...
void LPUART4_IRQHandler(void)//openart2
{
...
//        
//        gps_uart_callback();
//			bluetooth_debug_handler();
			openartmini_debug_handler();
    }
...
void LPUART6_IRQHandler(void)//蓝牙串口
{
...
void LPUART8_IRQHandler(void)//两车通信
{
...
void GPIO2_Combined_0_15_IRQHandler(void)
{
    ...
    if(exti_flag_get(C15))
    {
      exti_flag_clear(C15);
			system_delay_us(10000);
			if(~gpio_get_level(C15))
				rt_event_send(key_event, KEY_C15);// 清除中断标志位	
    }
    if(exti_flag_get(C14))
    {
      exti_flag_clear(C14);
			system_delay_us(10000);
			if(~gpio_get_level(C14))
				rt_event_send(key_event, KEY_C14);// 清除中断标志位
    }
    if(exti_flag_get(C13))
    {
      exti_flag_clear(C13);
			system_delay_us(10000);
			if(~gpio_get_level(C13))
				rt_event_send(key_event, KEY_C13);// 清除中断标志位
    }
    if(exti_flag_get(C12))
    {
			exti_flag_clear(C12);
			system_delay_us(10000);
			if(~gpio_get_level(C12))
				rt_event_send(key_event, KEY_C12);// 清除中断标志位
    }
}
