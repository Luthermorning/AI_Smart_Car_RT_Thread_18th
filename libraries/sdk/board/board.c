...
//void example_uartl_callback(LPUART_Type*base, lpuart_handle_t *handle, status_t status, void *userData)
//{
...
void rt_hw_console_output(const char *str)
{
    while(RT_NULL != *str)
    {
        if('\n' == *str)
        {
            uart_write_byte(UART_1, '\r');
        }
        uart_write_byte(UART_1, *str++);
    }
}
...
char rt_hw_console_getchar(void)
{
    uint32 dat;
    //等待邮件
    rt_mb_recv(uart_mb, (rt_ubase_t*)&dat, RT_WAITING_FOREVER);
